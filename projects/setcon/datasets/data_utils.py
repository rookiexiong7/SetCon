# ---- Merged SetCon dataset utilities ----
import copy
...
def setcon_collect_fn(
    ...
    """
    Collate function for SetCon segmentation datasets.

    Handles input_ids/labels, pixel_values, masks, and optional g_pixel_values.
    """
    ...
    for example in instances:
        ...
        if has_mask:
            if 'masks' in example.keys() and example['masks'] is not None:
                object_masks.append(example['masks'])  # List(BatchSize)[List(Token)[tensor(N, H, W)]]
...
class ConcatDatasetSetCon(TorchConcatDataset):
